...
class TrajectoryData:
    ...
    trajectory_segments: List[Tuple[int, int]] = field(default_factory=list)  # Список кортежей (start_index, end_index) для отдельных сегментов траектории
...
class StatisticsData:
    ...
    def to_dataframe_control(self) -> pd.DataFrame:
        ...
        data = {
            ...
            'dX': [self.control_min_dx, self.control_max_dx, self.control_mean_dx,
                   self.control_std_dx, self.control_mad_dx],
            'dY': [self.control_min_dy, self.control_max_dy, self.control_mean_dy,
                   self.control_std_dy, self.control_mad_dy],
            'dZ': [self.control_min_dz, self.control_max_dz, self.control_mean_dz,
                   self.control_std_dz, self.control_mad_dz],
            '2D_dist': [self.control_min_2d, self.control_max_2d, self.control_mean_2d,
                        self.control_std_2d, self.control_mad_2d],
            '3D_dist': [self.control_min_3d, self.control_max_3d, self.control_mean_3d,
                        self.control_std_3d, self.control_mad_3d]
        }
    ...
    def to_dataframe_check(self) -> pd.DataFrame:
        ...
        data = {
            'Метрика': ['Min', 'Max', 'Mean', 'Std Dev', 'MAD'],
            'dX': [self.check_min_dx, self.check_max_dx, self.check_mean_dx,
                   self.check_std_dx, self.check_mad_dx],
            'dY': [self.check_min_dy, self.check_max_dy, self.check_mean_dy,
                   self.check_std_dy, self.check_mad_dy],
            'dZ': [self.check_min_dz, self.check_max_dz, self.check_mean_dz,
                   self.check_std_dz, self.check_mad_dz],
            '2D_dist': [self.check_min_2d, self.check_max_2d, self.check_mean_2d,
                        self.check_std_2d, self.check_mad_2d],
            '3D_dist': [self.check_min_3d, self.check_max_3d, self.check_mean_3d,
                        self.check_std_3d, self.check_mad_3d]
        }
        return pd.DataFrame(data)
