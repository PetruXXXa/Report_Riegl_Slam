...
class PointSelector:
    ...
    def select_points(
        self, projects: List[ProjectData], project_type: str
    ) -> List[ControlPoint]:
        """
    ...
    def _select_multiple_msa_points(
                ...
                all_points_with_proj.append(
                    {
                        ...
                        "point_idx": len(
                            [
                                p
                                for p in all_points_with_proj
                                if p["project_idx"] == proj_idx
                            ]
                        ),
                    }
                )
    ...
    def _form_clusters(
        self, projects: List[ProjectData], all_points: List[Dict]
    ) -> List[List[Dict]]:
        """
        ...
        for i in range(n_projects):
            if not visited[i]:
                ...
                for proj_idx in cluster:
                    proj_points = [
                        p for p in all_points if p["project_idx"] == proj_idx
                    ]
    ...
    def _farthest_point_sampling(
        self, points: List[ControlPoint], gcp_indices: set, target_gcp: int
    ):
        """
...
class DataExtractor:
    ...
    def extract_project_data(
        self, project_path: str, project_name: str, project_type: str
    ) -> ProjectData:
        """
        Извлекает все данные из проекта (шаги 1-4)
        """
        project = ProjectData(
            name=project_name, path=project_path, project_type=project_type
        )
    ...
    def _extract_rs10_trajectory(self, project: ProjectData):
        ...
        traj_path = os.path.join(
            project.path, "AUTOSOLVE", "Scanner1", "slam_trajectory.txt"
        )
    ...
    def _extract_msa_control_points_raw(self, project: ProjectData):
        ...
        try:
            ...
            for idx, row in df.iterrows():
                name = (
                    str(row["name"]).strip() if "name" in row else f"Point_{idx}"
                )  # Убираем пробелы
                ...
                if transform_matrix is not None:
                    ...
                    transformed = self.converter.apply_transform(
    ...
    def _extract_rs10_control_points_raw(self, project: ProjectData):
        ...
        report_path = os.path.join(
            project.path, "AUTOSOLVE", "TGCPReport", "SLAM_Refine_Report.csv"
        )
        ...
        try:
            ...
            df = df[df[name_col] != ""]
            df = df[
                ~df[name_col].astype(str).str.contains("Мин|Макс|Среднее|Ср", na=False)
            ]
        ...
        try:
            ...
            for col_idx, col_name in enumerate(df.columns.get_level_values(1)):
                ...
                if "name" in col_str:
                    name_col = col_idx
                    print(
                        f"Найдена колонка имени: индекс {col_idx}, значение '{col_name}'"
                    )
                elif "easting" in col_str:
                    easting_col = col_idx
                    print(
                        f"Найдена колонка Easting: индекс {col_idx}, значение '{col_name}'"
                    )
                elif "northing" in col_str:
                    northing_col = col_idx
                    print(
                        f"Найдена колонка Northing: индекс {col_idx}, значение '{col_name}'"
                    )
                elif "height" in col_str:
                    height_col = col_idx
                    print(
                        f"Найдена колонка Height: индекс {col_idx}, значение '{col_name}'"
                    )
            ...
            if easting_col is None or northing_col is None or height_col is None:
                print(
                    f"Не все координатные колонки найдены: Easting={easting_col}, Northing={northing_col}, Height={height_col}"
                )
                return coords
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    try:
                        easting = (
                            round(float(easting_val), 3)
                            if pd.notna(easting_val)
                            else 0.0
                        )
                        northing = (
                            round(float(northing_val), 3)
                            if pd.notna(northing_val)
                            else 0.0
                        )
                        height = (
                            round(float(height_val), 3) if pd.notna(height_val) else 0.0
                        )

                        coords[name] = (easting, northing, height)
                        print(
                            f"Извлечены координаты для {name}: Easting={easting}, Northing={northing}, Height={height}"
                        )
                    except (ValueError, TypeError):
                        print(
                            f"Нечисловые значения для точки {name}: Easting={easting_val}, Northing={northing_val}, Height={height_val}"
                        )
                        continue
    ...
    def _extract_table_coordinates(self, project: ProjectData):
        ...
        if os.path.exists(html_path):
            try:
                ...
                # Проверяем структуру HTML отчета
                html_structure_status = self._check_html_structure(soup)
                project.html_structure_status = html_structure_status

                print(f"Статус структуры HTML: {html_structure_status}")

                # Ищем таблицу 2.4 (Control Points)
                print("Поиск таблицы 2.4 Control Points...")
                table_coords = self._extract_coordinates_from_table(
                    soup, "2.4 Control Points"
                )
                print(f"Из таблицы 2.4 извлечено координат: {len(table_coords)}")

                # Если есть таблица 2.5, тоже извлекаем (если структура позволяет)
                if html_structure_status in ["full", "partial"]:
                    print("Поиск таблицы 2.5 Check Points...")
                    table_coords_check = self._extract_coordinates_from_table(
                        soup, "2.5 Check Points"
                    )
                    if table_coords_check:
                        print(
                            f"Из таблицы 2.5 извлечено координат: {len(table_coords_check)}"
                        )
                        table_coords.update(table_coords_check)
                    else:
                        print("Таблица 2.5 не найдена или пуста")
                else:
                    print("Пропускаем таблицу 2.5 из-за неправильной структуры HTML")
                ...
                for point in project.control_points:
                    if point.name in table_coords:
                        point.x_table, point.y_table, point.z_table = table_coords[
                            point.name
                        ]
                        updated_count += 1
                        print(
                            f"Для точки {point.name} обновлены координаты: X={point.x_table}, Y={point.y_table}, Z={point.z_table}"
                        )
                    else:
                        print(
                            f"Точка {point.name} не найдена в HTML, используются исходные координаты"
                        )

                print(
                    f"Обновлено координат для {updated_count} из {len(project.control_points)} точек"
                )
                ...
                traceback.print_exc()
                project.html_structure_status = "error"
        else:
            print(
                f"HTML отчет не найден: {html_path}, используются исходные координаты"
            )
            project.html_structure_status = "no_html"
    ...
    def _check_html_structure(self, soup) -> str:
        """
        Проверяет структуру HTML отчета и возвращает статус:
        - 'full': полная структура (есть 2.4, 2.5, 4.4.1, 4.4.2 Check Points)
        - 'partial': частичная структура (есть 2.4 и 4.4.1, но нет или неправильные 2.5/4.4.2)
        - 'minimal': минимальная структура (только 2.4 и 4.4.1)
        - 'invalid': некорректная структура
        """
        try:
            ...
            print(f"Найдены заголовки в HTML: {headers[:20]}...")  # Ограничиваем вывод

            # Проверяем ключевые секции
            has_24 = any("2.4 Control Points" in h for h in headers)
            has_25 = any("2.5 Check Points" in h for h in headers)
            has_441 = any("4.4.1 Control Points" in h for h in headers)
            has_442_check = any("4.4.2 Check Points" in h for h in headers)
            has_442_any = any("4.4.2" in h for h in headers)  # Любая секция 4.4.2

            print(
                f"Секции найдены: 2.4={has_24}, 2.5={has_25}, 4.4.1={has_441}, 4.4.2 Check Points={has_442_check}, 4.4.2 любая={has_442_any}"
            )

            # Определяем статус
            if has_24 and has_441:
                if has_25 and has_442_check:
                    return "full"  # Полная структура
                elif has_25 or has_442_any:
                    return "partial"  # Частичная структура (есть что-то из 2.5/4.4.2)
                else:
                    return "minimal"  # Только обязательные секции
            else:
                return "invalid"  # Нет обязательных секций

        except Exception as e:
            print(f"Ошибка проверки структуры HTML: {e}")
            return "error"
    ...
    def _extract_msa_deviations(self, project: ProjectData):
        ...
        try:
            ...
            # Извлекаем из секции 4.4.1 (Control Points) - всегда должна быть
            stats_441, deviations_441 = self._extract_section_deviations(
                soup, "4.4.1 Control Points", has_force_match=True
            )
            if deviations_441 is not None:
                ...
                print(
                    "Предупреждение: секция 4.4.1 Control Points не найдена или пуста"
                )

            # Извлекаем из секции 4.4.2 (Check Points) - может отсутствовать
            # Пробуем разные варианты названий секции
            section_442_found = False
            section_442_variants = [
                "4.4.2 Check Points",
                "4.4.2 Check Points in CRS#2",
                "4.4.2 Check Points in a coordinate reference system",
            ]

            for section_variant in section_442_variants:
                stats_442, deviations_442 = self._extract_section_deviations(
                    soup, section_variant, has_force_match=False
                )
                if deviations_442 is not None:
                    section_442_found = True
                    for _, row in deviations_442.iterrows():
                        ...
                        print(f"Из секции {section_variant} загружена точка {name}")
                    break  # Используем первый найденный вариант

            if not section_442_found:
                print("Предупреждение: секция 4.4.2 Check Points не найдена")
                # Проверяем, есть ли вообще какая-то секция 4.4.2
                has_any_442 = False
                for tag in soup.find_all(["h3"]):
                    if "4.4.2" in tag.get_text():
                        has_any_442 = True
                        print(f"Найдена другая секция 4.4.2: {tag.get_text()}")
                        break

                if not has_any_442:
                    print("Секция 4.4.2 отсутствует в отчете")
            ...
            points_with_gcp_flags = 0

            for point in project.control_points:
                point_name = (
                    point.name.strip()
                )  # Убираем пробелы из имени точки проекта
                if point_name in all_deviations:
                    ...
                    point.dist_2d = (
                        round(dist_2d, 3) if dist_2d is not None else dist_2d
                    )
                    point.dist_3d = (
                        round(dist_3d, 3) if dist_3d is not None else dist_3d
                    )
                    ...
                    print(
                        f"Сопоставлена точка: {point.name} -> {point_name}, is_gcp={point.is_gcp}"
                    )
                else:
                    print(
                        f"Предупреждение: для точки '{point.name}' (нормализовано: '{point_name}') нет отклонений"
                    )

            print(
                f"Загружено отклонений для {points_with_deviations} из {len(project.control_points)} точек"
            )
            print(
                f"Установлены флаги is_gcp для {points_with_gcp_flags} точек из отчета"
            )

            # Сохраняем информацию о наличии контрольных точек
            project.has_check_points_in_report = len(check_points_from_html) > 0
    ...
    def _parse_deviations_table_new(self, table, has_force_match=True):
        ...
        try:
            ...
            headers = [
                th.get_text().strip().lower() for th in header_row.find_all(["th"])
            ]
            ...
            for i in range(2, len(rows)):
                ...
                if len(cells) == 0 or (
                    len(cells) == 1 and cells[0].get_text().strip() == ""
                ):
                    continue
                ...
                if len(cells) >= 7:
                    try:
                        ...
                        data_row = {
                            ...
                            "dX": round(float(row_data[3]), 3)
                            if len(row_data) > 3 and row_data[3] and row_data[3] != ""
                            else None,
                            "dY": round(float(row_data[4]), 3)
                            if len(row_data) > 4 and row_data[4] and row_data[4] != ""
                            else None,
                            "dZ": round(float(row_data[5]), 3)
                            if len(row_data) > 5 and row_data[5] and row_data[5] != ""
                            else None,
                            "dist": round(float(row_data[6]), 3)
                            if len(row_data) > 6 and row_data[6] and row_data[6] != ""
                            else None,
                        }
                        ...
                        if any(
                            v is not None
                            for v in [data_row["dX"], data_row["dY"], data_row["dZ"]]
                        ):
    ...
    def _extract_rs10_deviations(self, project: ProjectData):
        ...
        try:
            ...
            print(
                f"RS10: загружены отклонения для {points_with_deviations} из {len(project.control_points)} точек"
            )
    ...
    def _parse_msa_deviations_section(self, section):
        ...
        for i in range(2, len(rows)):
            ...
            try:
                ...
                print(
                    f"Извлечены отклонения для {name}: dX={dx}, dY={dy}, dZ={dz}, dist={dist}"
                )
    ...
    def finalize_point_selection(
        self, projects: List[ProjectData], project_type: str, gui_callback=None
    ) -> List[ProjectData]:
        """
        Финальный шаг: применяет алгоритм выбора опорных/контрольных точек
        Возвращает список проектов для дальнейшей обработки (проблемные могут быть исключены)

        Args:
            projects: Список проектов
            project_type: 'msa' или 'rs10'
            gui_callback: Функция обратного вызова для GUI (для показа диалогов)

        Returns:
            Список проектов для обработки (может быть пустым), или None при отмене
        """
        # Для RS10 всегда применяем алгоритм
        if project_type == "rs10":
            for project in projects:
                self._apply_point_selection(project, projects, project_type)
            return projects

        # Для MSA анализируем ситуацию
        print(f"\n=== Анализ структуры MSA проектов ===")

        # Проверяем структуру HTML отчетов
        projects_with_issues = []
        for project in projects:
            if project.project_type == "msa":
                print(
                    f"Проект {project.name}: статус HTML = {project.html_structure_status}, есть контрольные точки = {project.has_check_points_in_report}"
                )

                if (
                    project.html_structure_status in ["partial", "minimal", "invalid"]
                ) or not project.has_check_points_in_report:
        ...
        # Есть проблемные проекты
        if gui_callback:
            choice, cancelled_names = gui_callback(projects_with_issues)
        else:
            choice, cancelled_names = "keep", []

        if choice is None:
            return None

        # Исключаем отменённые проекты
        remaining = [p for p in projects if p.name not in cancelled_names]

        # Применяем выбранное решение к оставшимся проблемным проектам
        for project in remaining:
            if project in projects_with_issues:
                project.user_choice_for_incomplete = choice

                if choice == "keep":
                    print(f"Проект {project.name}: оставляем как есть (все опорные)")
                    for point in project.control_points:
                        if not point._is_gcp_set_from_report:
                            point.is_gcp = True
                            point._is_gcp_set_from_report = True

                elif choice == "algorithm":
                    print(f"Проект {project.name}: применяем алгоритм выборки")
                    self._apply_point_selection(project, [project], project_type)
            else:
                self._apply_point_selection(project, [project], project_type)

        gcps = [p for pj in remaining for p in pj.control_points if p.is_gcp]
        checks = [p for pj in remaining for p in pj.control_points if not p.is_gcp]
        print(f"Итоговое распределение: опорных - {len(gcps)}, контрольных - {len(checks)}")

        return remaining

    def _apply_point_selection(self, project, projects, project_type):
        """Применяет алгоритм выбора к одному проекту"""
        result = self.selector.select_points([project], project_type)
        for i, point in enumerate(project.control_points):
            if i < len(result):
                point.is_gcp = result[i].is_gcp
                point._is_gcp_set_from_report = True
    ...
    def combine_projects(self, projects: List[ProjectData]) -> ProjectData:
        ...
        combined = ProjectData(
            name="Combined", path="", project_type=projects[0].project_type
        )
        ...
        combined.total_trajectory_length = sum(
            p.total_trajectory_length for p in projects
        )
