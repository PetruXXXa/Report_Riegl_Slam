...
class HTMLPreviewFrame(tk.Frame):
    ...
    def save_html(self):
        ...
        if file_path:
            try:
                ...
                if messagebox.askyesno("Открыть отчет", "Открыть сохраненный отчет в браузере?"):
    ...
    def refresh_preview(self):
        ...
        if self.main_gui and hasattr(self.main_gui, 'regenerate_report_with_current_settings'):
...
class MSAReportGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Генератор отчетов MSA")
    ...
    def create_control_panel(self, parent):
        ...
        ttk.Button(frame, text="Выбрать директорию",
                  command=self.select_directory).pack(fill=tk.X, pady=2)
        
        ttk.Button(frame, text="Сгенерировать отчет",
                  command=self.generate_report).pack(fill=tk.X, pady=2)
        
        ttk.Button(frame, text="Экспорт в PDF",
                  command=self.export_to_pdf).pack(fill=tk.X, pady=2)
        
        ttk.Button(frame, text="Экспорт в Word",
                  command=self.export_to_word).pack(fill=tk.X, pady=2)
        
        ttk.Checkbutton(frame, text="Использовать карты OSM",
                       variable=self.use_osm_maps).pack(fill=tk.X, pady=2)
    ...
    def create_type_selector(self, parent):
        ...
        ttk.Radiobutton(frame, text="Riegl (MSA)",
                       variable=self.project_type_var,
                       value="msa").pack(anchor=tk.W, pady=2)
        
        ttk.Radiobutton(frame, text="RS10",
                       variable=self.project_type_var,
                       value="rs10").pack(anchor=tk.W, pady=2)
    ...
    def create_project_list(self, parent):
        ...
        scrollbar = ttk.Scrollbar(self.projects_frame, orient="vertical", command=canvas.yview)
        ...
        ttk.Button(select_frame, text="Выбрать все",
                  command=self.select_all).pack(side=tk.LEFT, padx=(0, 5), expand=True, fill=tk.X)
        ttk.Button(select_frame, text="Снять все",
                  command=self.deselect_all).pack(side=tk.LEFT, expand=True, fill=tk.X)
    
    def create_preview_area(self, parent):
        """Создает область предпросмотра"""
        preview_container = ttk.LabelFrame(parent, text="Предпросмотр отчета", padding=(5, 5))
    ...
    def show_initial_preview(self):
        ...
        initial_text = "=" * 60 + "\n"
        initial_text += "     ГЕНЕРАТОР ОТЧЕТОВ MSA\n"
        initial_text += "=" * 60 + "\n\n"
    ...
    def on_selection_change(self):
        ...
        if selected:
            self.status_var.set(f"Выбрано проектов: {len(selected)}. Загрузка данных...")
            self.preview_frame.clear()
            threading.Thread(target=self._load_projects_thread, args=(selected,), daemon=True).start()
        else:
            self.show_initial_preview()
    ...
    def _load_projects_thread(self, selected_projects):
        ...
        try:
            ...
            for proj_name in selected_projects:
                ...
                project = self.extractor.extract_project_data(proj_path, proj_name, proj_type)
                projects.append(project)
            
            self.update_queue.put(('status', "Выбор опорных и контрольных точек..."))
            # Применяем алгоритм выбора опорных/контрольных точек
            self.extractor.finalize_point_selection(projects, self.project_type_var.get())
            
            # Теперь для каждого проекта рассчитываем статистику и строим графики
            for i, project in enumerate(projects):
                self.update_queue.put(('status', f"Обработка проекта {i+1}/{len(projects)}..."))
                ...
                # Строим карту
                self.map_builder.create_map(project, use_osm=self.use_osm_maps.get(), width=16, height=10)
            ...
            if len(projects) > 1:
                ...
                self.map_builder.create_map(combined, use_osm=self.use_osm_maps.get(), width=16, height=10)
            ...
            html_content = self.filler.fill_report(
                ...
                use_html_tables=self.use_html_tables.get()
            )
    ...
    def regenerate_report_with_current_settings(self):
        ...
        self.map_builder.create_map(self.current_project, use_osm=self.use_osm_maps.get(), width=16, height=10)
    ...
    def export_to_pdf(self):
        ...
        if file_path:
            ...
            def export_thread():
                success = self.exporter.export_to_pdf(self.preview_frame.html_content, file_path)
                if success:
                    self.update_queue.put(('status', f"PDF сохранен: {os.path.basename(file_path)}"))
                    if messagebox.askyesno("Готово", "PDF сохранен. Открыть файл?"):
                        webbrowser.open(file_path)
                else:
                    self.update_queue.put(('status', "Ошибка экспорта в PDF"))
                    messagebox.showerror("Ошибка", "Не удалось создать PDF. Установлен ли wkhtmltopdf?")
    ...
    def export_to_word(self):
        ...
        if file_path:
            ...
            def export_thread():
                ...
                success = self.exporter.export_to_word_enhanced(self.preview_frame.html_content, file_path)
                if success:
                    self.update_queue.put(('status', f"Документ сохранен: {os.path.basename(file_path)}"))
                    if messagebox.askyesno("Готово", "Документ сохранен. Открыть файл?"):
                        webbrowser.open(file_path)
                else:
        ...
        try:
            while True:
                ...
                if msg[0] == 'scan_complete':
                    ...
                    self.update_project_display()
                    self.status_var.set(f"Найдено MSA: {len(self.msa_projects)}, RS10: {len(self.rs10_projects)}")
