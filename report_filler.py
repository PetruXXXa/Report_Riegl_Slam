...
class ReportFiller:
    ...
    def fill_report(
        ...
        # Формируем карту
        map_html = self._format_map_image(
            map_image_path or project.map_image_path, project
        )

        # Формируем таблицы точек
        points_tables = self._format_points_tables(project, use_html_tables)

        # Формируем таблицы отклонений для опорных точек
        control_deviations = self._format_deviations_table(
            project.get_gcps(), "Опорные точки"
        )

        # Формируем таблицы статистики для опорных точек
        control_stats = self._format_stats_table(
            project.statistics.to_dataframe_control(), "Опорные точки"
        )

        # Формируем график для опорных точек
        ...
        # Формируем секции для контрольных точек (если они есть)
        if has_checkpoints:
            check_deviations = self._format_deviations_table(
                checkpoints, "Контрольные точки"
            )
            check_stats = self._format_stats_table(
                project.statistics.to_dataframe_check(), "Контрольные точки"
            )
            check_graph = self._format_graph_image(project.check_graph_path)
        else:
            # Если контрольных точек нет, создаем заглушки
            check_deviations = "<p><i>Контрольные точки отсутствуют в отчете</i></p>"
            check_stats = "<p><i>Статистика по контрольным точкам недоступна</i></p>"
            check_graph = "<p><i>График отклонений контрольных точек недоступен</i></p>"

        # Добавляем информацию о структуре отчета (если есть)
        structure_info = ""
        if (
            hasattr(project, "html_structure_status")
            and project.html_structure_status != "full"
        ):
            status_messages = {
                "partial": "Частичная структура (не все секции найдены)",
                "minimal": "Минимальная структура (только обязательные секции)",
                "invalid": "Некорректная структура",
                "no_html": "HTML отчет отсутствует",
                "error": "Ошибка анализа HTML",
            }
            status_msg = status_messages.get(
                project.html_structure_status, project.html_structure_status
            )

            if (
                hasattr(project, "user_choice_for_incomplete")
                and project.user_choice_for_incomplete
            ):
                choice_messages = {
                    "skip_checks": "Пользователь выбрал пропуск контрольных точек",
                    "algorithm": "Контрольные точки выбраны алгоритмически",
                }
                choice_msg = choice_messages.get(project.user_choice_for_incomplete, "")
                if choice_msg:
                    status_msg += f" ({choice_msg})"

            structure_info = f"""
            <div class="structure-info">
                <h4>Информация о структуре отчета:</h4>
                <p><strong>Статус:</strong> {status_msg}</p>
                <p><strong>Контрольные точки:</strong> {"присутствуют" if has_checkpoints else "отсутствуют"}</p>
            </div>
            """
    ...
    def _create_html_preview(self, parent, html_content):
        ...
        open_btn, save_btn = self._create_toolbar_buttons(
            toolbar, html_content, temp_path
        )

        # Кнопка обновить для HTML предпросмотра
        refresh_btn = tk.Button(
            toolbar, text="Обновить", command=lambda: html_frame.load_html(html_content)
        )
    ...
    def _create_fallback_preview(self, parent, html_content):
        ...
        text_widget = scrolledtext.ScrolledText(
            text_frame, wrap=tk.WORD, font=("Courier", 10), bg="white"
        )
    ...
    def _create_fallback_preview_content(self, parent, html_content):
        ...
        # Текстовый виджет для упрощенного просмотра
        text_widget = scrolledtext.ScrolledText(
            content_frame, wrap=tk.WORD, font=("Courier", 10), bg="white"
        )
        text_widget.pack(fill=tk.BOTH, expand=True)
    ...
    def _create_preview_text(self, html_content: str) -> str:
        ...
        stats_match = re.findall(
            r'<span class="text-stat-label">(.*?)</span>\s*<span class="text-stat-value">(.*?)</span>',
            html_content,
        )
        if stats_match:
    ...
    def _format_points_tables(self, project: ProjectData, use_html: bool) -> str:
        ...
        if project.get_checkpoints():
            df = self._create_points_dataframe(
                project.get_checkpoints(), is_control=False
            )
    ...
    def _format_stats_table(self, df: pd.DataFrame, title: str) -> str:
        ...
        df_display["Метрика"] = (
            df_display["Метрика"].map(metric_map).fillna(df_display["Метрика"])
        )
